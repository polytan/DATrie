...
func (_ *ACTrie) g(s *node, a byte) *node {
	if c, ok := s.children[a]; ok {
		return c
	}
...
func (this *ACTrie) f(s *node) *node {
	return this.nodeValue[s.number].failure
}

func (this *ACTrie) out(s *node) []string {
	return this.nodeValue[s.number].output
}

func (this *ACTrie) sf(s *node, t *node) {
...
func (this *ACTrie) BuildTrie(arr []string) {
	...
	Q := []*node{}
	for _, q := range this.nodeStruct.root.children {
		Q = append(Q, q)
	}
...
func (this *ACTrie) Search(str string) interface{} {
	return this.nodeStruct.Search(str)
}

func (this *ACTrie) SearchDebug(str string) interface{} {
